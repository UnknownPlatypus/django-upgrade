...
def _is_daily_or_more_timedelta(node: ast.expr, state: State) -> bool:
    return (
        isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and is_name_attr(
        ...
        and len(node.args) in {0, 1, 7}
        and all(kw.arg in {"days", "weeks"} for kw in node.keywords)
    )
    ...
    if (
        ...
        and (
            (
                ...
                and _is_daily_or_more_timedelta(node.func.value.right, state)
            )
            or (
                ...
                and (
                    ...
                    or (
                        _is_timezone_localtime(node.func.value.right, state)
                        and _is_daily_or_more_timedelta(node.func.value.left, state)
                    )
                )
            )
        )
